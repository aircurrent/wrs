...
def build_cartesian_traj_for_arm(robot_dual: GP7_Dual,
    ...
    if arm == "lft":
        robot_dual.use_lft()
        dR_world = dR_world_lft
    elif arm == "rgt":
        robot_dual.use_rgt()
        dR_world = dR_world_rgt
    else:
        raise ValueError("arm must be 'lft' or 'rgt'")
...
if __name__ == "__main__":
    ...
    dp_rgt = np.array([-0.2357, -0.1536, -0.9330])
    dR_world_lft = rm.rotmat_from_axangle([0, 0, 1], np.deg2rad(0))
    # step 1: 使picking robot移动到hairpin的位置准备抓取
    step1_rgt_dRy = rm.rotmat_from_axangle([0, 1, 0], np.deg2rad(90))
    step1_rgt_dRz = rm.rotmat_from_axangle([0, 0, 1], np.deg2rad(-90))
    dR_world_rgt = step1_rgt_dRz @ step1_rgt_dRy  # 先Y后Z（作用顺序：右边先作用）
